...
test_estimates_all = mu_simulations %>%
  group_by(mu_value) %>%
  summarise(avg_estim = mean(estimate))
...
test_est_combine =
  ggplot(data = test_estimates_all, aes(x = mu_value, y = avg_estim)) +
    geom_point() +
    geom_smooth() +
    geom_point(data = test_estimates_reject, color = "green") +
    geom_smooth(data = test_estimates_reject, color = "green")
...
test_estimates_combine =
  ...
  pivot_longer(
    avg_estim_all:avg_estim_reject,
    names_to = "sample",
