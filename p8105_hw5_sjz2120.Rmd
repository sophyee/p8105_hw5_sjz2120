...
unsolved_balt = prop.test(x = balt_unsolved, n = balt_total) %>%
                broom::tidy()
```
...
The resulting dataframe with estimated proportions and CIs of unsolved homicides for each city is saved as `unsolved_all`.

```{r estim-all-cities}
city_unsolved = homicides_by_city %>% pull(n_unsolved)
city_total = homicides_by_city %>% pull(n_homicides)

unsolved_all = prop.test(x = city_unsolved,
                         n = city_total) %>%
                broom::tidy()
