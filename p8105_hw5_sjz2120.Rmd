...
Now, let's use this function and repeat the above simulation process for any μ (in this case, let's look at μ={0, 1, 2, 3, 4, 5, 6}):

```{r simulations-diffr-mu}
# Let's use simulate() and repeat this process 5000 times for a range of mu_values
mu_simulations = 
  tibble(mu_value = c(0, 1, 2, 3, 4, 5, 6)) %>%
  mutate(output = map(.x = mu_value, ~rerun(5000, simulate(mu = mu_value))),
         estimates_table = map(output, bind_rows)) %>%
  select(-output) %>%
  unnest(estimates_table)
...
```{r}
# Let's add a variable to `mu_simulations` called `reject_null`, which records whether the resulting `p.value` is < 0.05 (i.e. the null hypothesis is rejected)
mu_simulations = mu_simulations %>%
  mutate(reject_null = ifelse(p.value < 0.05, TRUE, FALSE))

mu_simulations %>%
  group_by(mu_value) %>%
  summarise(reject_null_prop = reject_null / n())
```
```{r}
